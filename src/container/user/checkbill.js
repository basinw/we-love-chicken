...
class CheckBill extends React.Component {
    ...
    componentWillMount = async () => {
        ...
        let data = await instance.get(`/bill/${billid}/total`)
            .then(resp => resp.data)
        if(data.status){
            ...
            // console.log(bill)
            let time = `${t.getDate()} ${allMonth[t.getMonth()]} ${t.getFullYear()}, ${this.twoDigit(t.getHours())}:${this.twoDigit(t.getMinutes())}:${this.twoDigit(t.getSeconds())}`
            t = new Date()
            let now = `${t.getDate()} ${allMonth[t.getMonth()]} ${t.getFullYear()}, ${this.twoDigit(t.getHours())}:${this.twoDigit(t.getMinutes())}:${this.twoDigit(t.getSeconds())}`
            this.setState({
                bill: data.bill,
        ...
        <div>
            <Body >
                <div className='container' style={{paddingTop: '50px'}}>
                    <Banner>
                        Please contact staff for check the bill
                    </Banner>
                    <Row className="row" >
                        <div className="col-6 bd text-right">
                            branch
                        </div>
                        <div className="col-6">
                            KMUTT
